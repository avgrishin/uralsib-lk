            ...
            table.case-table()
                caption.case-table__title
                    ...
                    drop-down.dropdown_trans.dropdown_trans.g-f_r.g-fs_05.g-mt_1_m_xs.g-hidden.g-show_xs.g-mr_1_m_xst(name="Тип операции")
                        .dropdown__item
                            button.g-btn-txt(@click.prevent="filterOperName = '*'") Все операции
                        .dropdown__item(v-for="(item, index) in filters.type", :key="index")
                            button.g-btn-txt(@click.prevent="filterOperName = item" v-html="item")
                thead
                    tr
                        th.case-table__th-name
                            drop-down.dropdown_trans.case-table__sort(name="Наименование фонда")
                                .dropdown__item
                                    button.g-btn-txt(@click.prevent="filterPortfolio = '*', selected[0] = null", :class="{'active': selected[0] == null}") Все фонды
                                .dropdown__item(v-for="(item, index) in filters.name", :key="index")
                                    button.g-btn-txt(@click.prevent="filterPortfolio = item, selected[0] = index", :class="{'active': index == selected[0] }", v-html="item")
                        th Дата#[br]проведения#[br]операции
                        th
                            drop-down.dropdown_trans.dropdown_trans.case-table__sort(name="Тип операции")
                                .dropdown__item
                                    button.g-btn-txt(@click.prevent="filterOperName = '*', selected[1] = null", :class="{'active': selected[1] == null}") Все операции
                                .dropdown__item(v-for="(item, index) in filters.type", :key="index")
                                    button.g-btn-txt(@click.prevent="filterOperName = item, selected[1] = index", :class="{'active': index == selected[1] }" v-html="item")
                ...
                tbody.case-table__style-icon_xs(v-for="(operation, index) in operationsFiltered" :key="index" :class="{'case-table__style-icon_open': isRowOpen(index) }")
                    tr.case-table__tr-md(@click.prevent="jsToggleTable(index)", :class="{'case-table__tr-md_open': isRowOpen(index) }")
                            ...
                            .td__name(:class="'td__name_'+index")
                                .case-table__icon-wrap
                                    div
                                        ...
                                        | #[br.g-hide_xs]
                                        span(v-html="operation.portfolio")
                        ...
                        td.case-table__td-md(title="Фонд на паи которого произведен обмен")
                            div(v-if="operation.exchPortfolio")
                                span.g-hide_xs УРАЛСИБ #[br.g-hide_xs]
                                span(v-html="operation.exchPortfolio")
                            div(v-else v-html="'–'")
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                ...
                axios.post('/reports/ClientPifOperations', body)
                .then(({ data: data }) => {
                    this.operations = this.convertData(data.items);
                    this.loaded = true;
                });
            },
            convertData(operations) {
                return operations.map(item => {
                    ...
                    item.commis =  item.commis ? this.formatCurrency(Math.abs(item.commis)) : '';
                    item.price =  item.price ? this.formatCurrency(Math.abs(item.price)) : '';
                    item.exchPrice =  item.exchPrice ? this.formatCurrency(Math.abs(item.exchPrice)) : '';
                    if (item.date) {
            ...
            showPopup(operation) {
                window.events.$emit('show_popup', ['preview-operation', { ...operation }]);
            },
        ...
        computed: {
            operationsFiltered() {
                ...
                return data.filter(item => {
                    if (this.filterPortfolio == '*') return true;

                    return item.portfolio == this.filterPortfolio;
                }).filter(item => {
                    if (this.filterOperName == '*') return true;

                    return item.operName == this.filterOperName;
                }).filter(item => {
                    if (this.filterExchange == '*') return true;

                    return item.exchPortfolio == this.filterExchange;
                });
            },
            filters() {
                let data = _.cloneDeep(this.operations);
                ...
                return {
                    name: _.uniq(data.map(item => item.portfolio)).sort(),
                    type: _.uniq(data.map(item => item.operName)).sort(),
                    exchange: _.uniq(data.map(item => item.exchPortfolio)).filter(item => item).sort()
                }
            }
        }
    }
